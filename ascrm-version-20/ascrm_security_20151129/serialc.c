...
int  OpenPort_C(const int port_nmbr, int* const h_commport)
{
	...
	int rtcode=1;
        if(port_nmbr <0 )
        {
              writeFileLog("[OpenPort()] com port number is less than zero.");
	...
	char com_port[30];
        ...
        {
             writeFileLog("[OpenPort()] sprintf return 0 on failure for com port.");
             return(CCTALK_FAIL); //Error Code
  	...
  	if(*h_commport < 0)
	{
            writeFileLog("[OpenPort()] open() return failure.");
            return(CCTALK_FAIL); //Error Code
	}
        ...
        */       
        ...
        if(-1==rtcode)
        {
            writeFileLog("[OpenPort()] fcntl() return failure.");
            return(CCTALK_FAIL);
        }
        ...
        if(-1==rtcode)
        {
            writeFileLog("[OpenPort()] tcgetattr() return failure.");
            return(CCTALK_FAIL);
        }
        ...
        */
        ...
        if(-1==rtcode)
        {
             writeFileLog("[OpenPort()] cfsetispeed() return failure.");
             return(CCTALK_FAIL);
        }
        rtcode=cfsetospeed(&portParams, B9600);
        if(-1==rtcode)
        {
            writeFileLog("[OpenPort()] cfsetospeed() return failure.");
            return(CCTALK_FAIL);
        }
	...
	//
       ...
       */
        ...
        if(-1==rtcode)
        {
            writeFileLog("[OpenPort()] tcsetattr() return failure.");
            return(CCTALK_FAIL);
        }   
 
        //If all settings done return SUCCESS
        writeFileLog("[OpenPort()] Port open success.");
        return(CCTALK_SUCCESS);


}//int  OpenPort_C(const int port_nmbr, int* const h_commport) end
