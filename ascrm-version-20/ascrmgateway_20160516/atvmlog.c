...
static int WriteLogInConsole(char *str)
{
		...
		if( NULL == g_lib_handle )
		{
				   ...
				   if (NULL==g_lib_handle) 
				   {
				       printf("\n[WriteLogInConsole()] Get so memory pointer failed");
				       return false; //failed
				   }
				   else
				   {
				       //success
		                       //printf("\n[WriteLogInConsole() if block] Get so memory pointer successfully done");
		...
		if(NULL==g_Ascrm_writeFileLog)
		{
		      ...
		      {
			    printf("\n[WriteLogInConsole() if block] Get so memory function pointer failed");
		            return 0;
		      }
		      else
		      {
			   //printf("\n[WriteLogInConsole() if block] Get so memory function pointer get success");
		      }
		...
		{
		    //printf("\n[WriteLogInConsole() if block] Get so memory function pointer already done");
