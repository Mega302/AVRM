...
int  OpenPort_C(const int port_nmbr, int* const h_commport)
{
        ...
        if(port_nmbr <0 )
        {
             AVRM_Currency_writeFileLog("[OpenPort_C()] Port Number error",FATAL);
        ...
        memset(log,'\0',100);
        sprintf(log,"[OpenPort_C()] OpenPort_C : %s .",com_port);
        ...
        {
             AVRM_Currency_writeFileLog("[OpenPort_C()] Write Failed",FATAL);
             return(CCTALK_FAIL); //Error Code
        }
        *h_commport = open(com_port,O_RDWR | O_NOCTTY | O_NDELAY);
		...
		{
			    memset(log,'\0',100);
				sprintf(log,"[OpenPort_C()] Open Port Failed : %s .",com_port);
				AVRM_Currency_writeFileLog(log,FATAL);
				return(CCTALK_FAIL); //Error Code
		}
        ...
        */       
        ...
        {
			memset(log,'\0',100);
		    sprintf(log,"[OpenPort_C()] Open Port Set Failed due to fcntl call: %s .",com_port);
	        AVRM_Currency_writeFileLog(log,FATAL);
            return(CCTALK_FAIL);
        }
        ...
        */
        rtcode=cfsetispeed(&portParams, B38400);
        if(-1==rtcode)
        {
			 memset(log,'\0',100);
		     sprintf(log,"[OpenPort_C()] Open Port Baud Rate Write Set Failed : %s .",com_port);
			 AVRM_Currency_writeFileLog(log,FATAL);
             return(CCTALK_FAIL);
        }
        rtcode=cfsetospeed(&portParams, B38400);
        if(-1==rtcode)
        {
			memset(log,'\0',100);
		    sprintf(log,"[OpenPort_C()] Open Port Baud Rate Read Set Failed : %s .",com_port);
			AVRM_Currency_writeFileLog(log,FATAL);
            return(CCTALK_FAIL);
        }
	...
	//
        ...
        portParams.c_cc[VTIME] = 10; //Wait for 1 Second
	...
	//
       ...
       */
        ...
        {
			memset(log,'\0',100);
		    sprintf(log,"[OpenPort_C()] Open Port Set Failed due to parameter settings: %s .",com_port);
	        AVRM_Currency_writeFileLog(log,FATAL);
            return(CCTALK_FAIL);
        }   
        ...
        memset(log,'\0',100);
		sprintf(log,"[OpenPort_C()] Open Port Set Success: %s .",com_port);
        AVRM_Currency_writeFileLog(log,INFO);
...
}//int  ClosePort_C(const int h_commport) end
...
int  FlushSerialPortOutBuff_C(const int SerialPortHwnd)
{
    ...
    if(0==tcflush(SerialPortHwnd,TCOFLUSH)) {//Clears the output buffer (if the device driver has one)
		AVRM_writeFileLogV2("[FlushSerialPortOutBuff_C()]  CCTALK Port Input Buffer Cleared.",INFO,"API");
	    return CCTALK_SUCCESS;
	}
    else{
		AVRM_writeFileLogV2("[FlushSerialPortOutBuff_C()]  CCTALK Port Input Buffer Not Cleared.",INFO,"API");
	    return CCTALK_FAIL;
	}
}
...
int FlushSerialPortInBuff_C(const int SerialPortHwnd)
{
	...
	if(0==tcflush(SerialPortHwnd,TCIFLUSH)) {//Clears the input buffer 
		   AVRM_writeFileLogV2("[FlushSerialPortInBuff_C()]  CCTALK Port Output Buffer Cleared.",INFO,"API");
           return CCTALK_SUCCESS;
     }      
      else
	  {    
		   AVRM_writeFileLogV2("[FlushSerialPortInBuff_C()]  CCTALK Port Output Buffer Not Cleared.",INFO,"API");
		   return CCTALK_FAIL;
	  }
