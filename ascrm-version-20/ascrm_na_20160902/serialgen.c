...
*/
int  GenricOpenPort(const int port_nmbr, int* const h_commport)
{
	int rtcode=1;
        if(port_nmbr <0 )
        return(SERIALB2B_FAIL); //Error Code
...
*/
int  GenricClosePort(const int h_commport)
{
...
*/
int GenricSendSingleByteToSerialPort(const int h_commport, const unsigned char bdata)
{
	if(h_commport < 1)
        return SERIALB2B_FAIL; //return Error
...
*/
int GenricReceiveSingleByteFromSerialPort(const int h_commport, unsigned char* const rbyte,int* const totalByteIn)
{
	...
	*totalByteIn = read(h_commport,&rxByte,1);
	...
	if(*totalByteIn > 0)
	{
		  ...
		  return(SERIALB2B_SUCCESS);                   // Successfully Received data byte from port.
	}

	*totalByteIn = 0;
	return(SERIALB2B_SUCCESS);                            // atleast port had not any error.
}


//Clear Transmit Buffer of Pc Serail Port
int  GenricClearTransmitBuffer(const int SerialPortHwnd)
{
    if(SerialPortHwnd < 1 )
    return SERIALB2B_FAIL; //Error Code
...
int GenricClearReceiveBuffer (const int SerialPortHwnd)
{
        if(SerialPortHwnd < 1 )
        return SERIALB2B_FAIL; //Error Code
