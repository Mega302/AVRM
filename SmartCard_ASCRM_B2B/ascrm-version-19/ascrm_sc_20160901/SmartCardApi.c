		    ...
		    if( true ==  IsAckReceieve() )
		    {
		         ...
		         if ( true == MUTEK_CommandReceieve(RecvPackt,totalByteToRecv))
		         {
		                  ...
		                  )
		                {
				             ...
				             writeFileLog("[GetSAMReaderStatus() Exit] Successfully get SAM status from mutek.");                           
                  ...
                  )
{
	      ...
	      memset(MTKStatus,0,6);
              ...
              if( g_ComHandle <= 0 )
              {
                    ...
                    ReplyDeviceStatus[0] = 1;

                    writeFileLog("[DeviceStatus_C() Exit] Communication Failure.");
...
/*              
              char buff[32]={0};
              sprintf(buff,"[DeviceStatus_C() rtcode = %d",rtcode);
	      writeFileLog(buff);
*/
...
*/
		...
		case 2:  //Card jam
			ReplyDeviceStatus[7] = 0x04; //Card jam
                ...
                case 3:  //Other error
			ReplyDeviceStatus[0] = 3; //Other error
                        break;

                case 4:  //Communication failure
			ReplyDeviceStatus[0] = 1; //Communication failure
                        return 1;
...
//                      Edited By: Abhishek Kole, 23-10-2017
        ...
        if( true == GetMTKSensorStatus( &ChanelStatus, &MTKSensorStatus ) )
        {
            ...
            if(  1 == (MTKSensorStatus & 0x01) ) //Sensor 1 status
            {
                writeFileLog("[RejectCard_c()] Sensor 1 Status Found Blocked As Card in Mouth:6 returned");
                // 6 = Card at Gate 
                return 6;  
            }//if(  1 == (MTKSensorStatus & 0x01) ) block end
		    ...
		    if( true ==  IsAckReceieve() )
		    {
		                ...
		                //Step 3: After Receieve ACK Go for read reply bytes
				             ...
				             ReplyAPDU[0] =  4 ;//Other Error
		                             *ReplyAPDULength = 1;
		                              writeFileLog("[ContactLessCardXChangeAPDU() Exit] Other Error Command Read Section.");
				              return 4;
                                }
		                else{
...
bool IsAckReceieve()
{
		...
		{
                      writeFileLog("[SendAckToMutek()] Receieve ACK Byte Failed.");
		      return false; 
...
bool MUTEK_CommandReceieve_V2(char *RecvPackt,unsigned int *rcvPktLen)
{
                      ...
                      if(0 == rtcode)
		      {
			       ...
			       writeFileLog("[MUTEK_CommandReceieve_V2() Timeout] Timeout or less data recieved");
                      ...
                      {  
                               ...
                               if( CalcultedBCC != (unsigned char)RecvPackt[*rcvPktLen-1] ){
                                    ...
                                    //Successfully Return
                                    SendAckToMutek();          
                                    writeFileLog("[MUTEK_CommandReceieve_V2() SuccessBlock] Successfully data recieved");
                                    return true;
                            ...
                            writeFileLog("[MUTEK_CommandReceieve_V2()] other error");
                            return false;
