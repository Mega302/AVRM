...
int IsCardRemoved_c(int Timeout)
{
        ...
        printf("\n[IsCardRemoved_c()] Going to send Current Report status of st0, st1, st2");
        bcc = getBcc(9,g_getStatus);
        g_getStatus[8] = bcc;
        printf("\n[IsCardRemoved_c()] bcc value is 0x%xh",bcc);
        ...
        for(i=0;i<9;i++)
	{
		 printf("\n[IsCardRemoved_c()] Report Status Command[%d] = 0x%xh",i,g_getStatus[i]);
		 send_rvalue = SendSingleByteToSerialPort(h_commport,g_getStatus[i]);
                 if(0 == send_rvalue) 
                 {
                    printf("\n[IsCardRemoved_c()] Failed to Send Report status Command ");
                    /* 2 = Communication Failure */
                    return 2;
                 }
	}

        printf("\n[IsCardRemoved_c()] Status Command send Successfully");
        printf("\n[IsCardRemoved_c()] Now Going to read Acknowledgement");
        ...
        printf("\n[IsCardRemoved_c()] Receive packet status = %d and Length = %d",rtcode,rcvPktLen);
        ...
        {
                 printf("\n[IsCardRemoved_c()] Acknowledgement against Status Command[0] = 0x%xh.\n",rcvPkt[0]);
        }
        ...
        if(0x06 == rcvPkt[0])
        {
                      printf("\n[IsCardRemoved_c()] Acknowledgement Received");
                      totalByteToRecv = 12;
                      memset(rcvPkt,'\0',24);
                      ...
                      if(0 == rtcode)
		      {
				 ...
				 return 2;
	              ...
	              {
				printf("\n[IsCardRemoved_c()] Receive packet Command Reply Data[%d] = 0x%xh",i,rcvPkt[i]);
                                /* Card Status Code st0 */
                                if(7 == i)
                                {
		                        if(0x30 == rcvPkt[i])
		                        {
                                           printf("\n[IsCardRemoved_c()] No Card in MTK-571");
                                            
		                        }
                                        else if(0x31 == rcvPkt[i])
		                        {
                                           printf("\n[IsCardRemoved_c()] One Card in Gate");
                                           /* 0 = Not Removed */
                                           oneCradInGate = 1;
		                        ...
		                        {
                                           printf("\n[IsCardRemoved_c()] One Card on RF/IC Card Position");
		                        ...
		                        if(0x30 == rcvPkt[i])
		                        {
                                           printf("\n[IsCardRemoved_c()] No Card in Stacker");
		                        }
                                        else if(0x31 == rcvPkt[i])
		                        {
                                           printf("\n[IsCardRemoved_c()] Few Card in Stacker");
		                        }
                                        else if(0x32 == rcvPkt[i])
		                        {
                                           printf("\n[IsCardRemoved_c()] Enough Card in the Box");
		                        }   
                                } 
		                        ...
		                        if(0x30 == rcvPkt[i])
		                        {
                                           printf("\n[IsCardRemoved_c()] Error Card bin Not Full");
		                        }
                                        else if(0x31 == rcvPkt[i])
		                        {
                                           printf("\n[IsCardRemoved_c()] Error Card Bin Full");
                      ...
                      if(0 == send_rvalue) 
		      {
		            printf("\n[IsCardRemoved_c()] Failed to send total Acknowledgement Command ");
                            /* 2 = Communication Failure */
                            return 2;
		      }
                      if(1 == oneCradInGate)
       ...
       {
             printf("\n[IsCardRemoved_c()] Nak Reply Received");
             /* 4 = Other Error */
             return 4;  
       }
