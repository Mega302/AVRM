...
int  OpenPort(const int port_nmbr, int baudrate,int* const h_commport)
{
	...
	//
	 ...
	 //
	 portParams.c_cc[VMIN]  = 0; // read doesn't block
	     ...
	     {        
				               ...
				               break;
                                    ...
                                    else if( (0xF2 == rcvPkt[0]) && (MTK_DEVICE_ADDRESS == rcvPkt[1]) && (byteRcvdCounter>=4) )
                                    {
                                              ...
                                              {     
                                                      ...
                                                      sprintf(log,"[statusRead() Text length calculate block] RceieveTextLength=%d and byteRcvdCounte=%d.",RceieveTextLength,byteRcvdCounter);
		                                      ...
		                                      sprintf(log,"[statusRead() Text length calculate block ] Expected byte receieve in %d second.",diffts.tv_sec);
			 ...
			 //if(  (byteRcvdCounter == 0)  &&  (diffts.tv_sec >= 1 )  )
                         if( diffts.tv_sec >= MAX_WAIT_TIME_COMPORT  )
                         {
                                 ...
                                 //printf("\n[statusRead()] Expected Byte not received in %d seconds .",diffts.tv_sec);
                                 ...
                                 ) 
                                 {
                                   ...
                                   sprintf(log,"[statusRead()] Time out in %d seconds",diffts.tv_sec);
