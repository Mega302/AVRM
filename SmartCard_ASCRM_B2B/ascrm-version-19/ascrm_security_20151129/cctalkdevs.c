...
int IsCCTALKPortOpen()
{
	  ...
	  if(true == g_CCTALK_OpenFlag)
          {
                writeFileLog("[IsCCTALKPortOpen()()] CCTALK Port opened.");
                rtcode = 1;

          }
	  else if(false == g_CCTALK_OpenFlag)
          {
                writeFileLog("[IsCCTALKPortOpen()()] CCTALK Port is not opened.");
                rtcode = 0;
          }
...
int DeActivateCCTALKPort()
{
	   ...
	   if(SUCCESS==rtcode)
	   {
                      ...
                      writeFileLog("[DeActivateCCTALKPort()] Close CCTALK Port successed.");
...
static int MONEYCONTROL_CoinAcptr_statusRead(  unsigned char* const rcvPkt,
                                               unsigned int* const rcvPktLen)
{
	     ...
	     {        
			 ...
			 if(  (byteRcvdCounter < totalByteToRecv)  &&  
                              ( diffts.tv_sec >= CCTALK_REPLY_WAIT_TIME ) )
			 {
		             ...
		             sprintf(log,"[getSecuredState()] StatusByte: 0x%xh.", GetStatusReplyPackt[StatusBytePosition]);
                             ...
                             if(NULL!=doorOpenStatus)
                             {
                                   ...
                                   *doorOpenStatus= bottomdoor ;
                                   ...
                                   if( 0 == bottomdoor)
                                   {
                                        writeFileLog("[getSecuredState()] Top door close.");
                                   }
                                   else if( 1 == bottomdoor)
                                   {
                                        writeFileLog("[getSecuredState()] Top door open.");
                                   }
                             ...
                             else if(NULL == cashboxOpenStatus)
                             {
                                  ...
                                  writeFileLog("[getSecuredState()] Top Door get status not defined in parameter.");
                             }
                             ...
                             else if(NULL == cashboxOpenStatus)
                             {
                                    ...
                                    writeFileLog("[getSecuredState()] cashbox get status not defined in parameter.");
                             ...
                             else if(NULL == cashboxOpenStatus)
                             {
                                  ...
                                  writeFileLog("[getSecuredState()] Alarm get status not defined in parameter.");
                             }
...
}//void SendSoftwareversionCommand() end
...
int main()
{
		  ...
		  int portno=-1;
                  ...
                  SetupSecurityLogFile();

                  /////////////////////////////////////////////////////////////

		  printf("\n(main) Enter ASCRM Secuirty Serial Port Number :");
		  ...
		  while(1)
		  {
			...
			printf("\n 3.Cris API Style Vault Open.");
                        printf("\n 4.Security Status infinite Loop Style.");
                        printf("\n 5.UPS Status.");
                        printf("\n 6.UPS Battery Charge Status.");
                        printf("\n 7.Exit");
				   ...
				   if(ApiAllowdooropenExpirytime> 65535)
				   {
				      printf("\nwrong input expiray time max value is 65535 seconds.");
                                   ...
                                   if(ApiAllowdooropenExpirytime > 0)
			           {
                               ...
                               case  5: //Ups status  
                                         if( 0 == GetAPCServiceStatus() ) //main power supply on
                                         {
                                              //writeFileLog("[main()] Main AC Power Supply is on.");
                                         }
                                         else
                                         {
                                              //writeFileLog("[main()] UPS is on and MAIN AC power is down.");
                                         }
                                         break;  

                               case  6:  //Ups battery status  
                                         memset(log,'\0',200);
		                         //sprintf(log,"[main()] UPS Battery Charge: %d ", GetAPCBatteryCharge());
                                         //writeFileLog(log);
                                         break;  
