...
int GetDSCUPortNo(void)
{
	   ...
	   {
		return(GetIntKeyValue(IniAbsoluteFilePath,"ATVM_PORTS","TrapGatePort"));
	   } 
	   else
	   {
...
int GetElectronicsLockTime(void)
{
	   ...
	   rtcode=GetIniAbsoluteFilePath("ascrm.ini",IniAbsoluteFilePath);
	   if(READ_INI_SUCCESS==rtcode)
	   return(GetIntKeyValue(IniAbsoluteFilePath,"ATVM_ELECTRONICS_LOCK_TIME","LOCKTIME")); 
	   else
	   return READ_INI_FAIL;
...
int GetLogFileMode(char *logfilepath)
{
		     ...
		     rtcode=GetStringKeyValue(IniAbsoluteFilePath,"ASCRM_CURRENCY_LOGMODE","ENABLE",KeyValue);
...
int GetLogFilePath(char *logfilepath)
{
		     ...
		     rtcode=GetIniAbsoluteFilePath("ascrm.prop",IniAbsoluteFilePath);
		     if(READ_INI_FAIL==rtcode)
		     return READ_INI_FAIL;
